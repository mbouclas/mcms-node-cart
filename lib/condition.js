...
Condition.prototype.apply = function(collection,target){
    ...
    target = collection[target] || collection[collection.target];
    this.subtotal = target;
    lo.reduce(this.actions,function(memo,action){
        target = (_this.result > 0) ? _this.result : target;
        _this.result = applyAction(collection,action,target);
    },0);
